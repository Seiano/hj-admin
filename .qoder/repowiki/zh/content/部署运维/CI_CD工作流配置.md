...
- [wiki-update.yml](file://.github/workflows/wiki-update.yml)
...
## 更新摘要
**变更内容**
- 新增Qoder Wiki自动更新工作流，实现Spec文档到项目Wiki的自动同步
- 增强change-impact和issue-refine工作流的提示词工程和上下文处理能力
- 完善了从需求管理到文档同步的完整自动化闭环
- 更新了架构总览和详细组件分析章节
...
## 简介
本仓库为"氢界大数据平台 — 运营管理后台"的前端工程，采用 Vite + React + Ant Design 技术栈。CI/CD 基于 GitHub Actions 实现，主要包含八个自动化流程：
...
- **Qoder 需求自动细化**：**增强** - 当Issue添加type:requirement标签时自动进行需求细化分析，优化了提示词工程和上下文处理
- **Qoder 变更影响评估**：**增强** - 当Issue添加type:change-request标签时自动评估变更影响范围，提升了智能识别能力
- **Qoder 自动 Changelog**：**新增** - 创建Release时自动生成版本更新日志
- **Qoder Wiki 自动更新**：**全新** - 当Spec文档合入main分支时自动更新项目Wiki，确保开发团队获取最新需求上下文

此外，工程内还包含构建、类型检查与 ESLint 校验脚本，便于本地与 CI 环境保持一致的开发体验。

## 项目结构
与 CI/CD 直接相关的目录与文件如下：
- .github/workflows：GitHub Actions 工作流定义（包含8个工作流）
...
H[".github/workflows/wiki-update.yml"]
end
...
N["docs/analysis/"]
O["docs/specs/"]
P["docs/templates/spec-template.md"]
Q["docs/pipeline.md"]
end
R["AGENTS.md"]
A --> I
...
- [change-impact.yml:1-44](file://.github/workflows/change-impact.yml#L1-L44)
- [auto-changelog.yml:1-35](file://.github/workflows/auto-changelog.yml#L1-L35)
- [wiki-update.yml:1-36](file://.github/workflows/wiki-update.yml#L1-L36)
...
- **Qoder 需求自动细化工作流（issue-refine.yml）**：**增强**
  ...
  - **增强特性**：优化了提示词工程，增加了AGENTS.md和项目规范文件的上下文引用，提升了需求细化的准确性和完整性
- **Qoder 变更影响评估工作流（change-impact.yml）**：**增强**
  ...
  - **增强特性**：改进了智能识别能力，能够更准确地提取关联Issue编号和分析Spec文档，提供更精准的影响评估
...
- **Qoder Wiki 自动更新工作流（wiki-update.yml）**：**全新**
  - 触发条件：向 main 或 master 分支推送时，且路径匹配 docs/specs/**
  - 运行环境：ubuntu-latest
  - 权限：id-token write、contents read、issues write
  - 步骤：检出仓库 → 调用 QoderAI/qoder-action@v0 → 读取最新的Spec文档，提取功能设计、交互流程、数据模型和验收标准，自动更新项目RepoWiki
...
## 架构总览
下图展示了八个工作流的触发点、执行环境与外部服务交互关系，以及从需求管理到Wiki同步的完整自动化流水线。
...
participant WU as "Qoder Wiki更新工作流<br/>wiki-update.yml"
...
Note over GH,WU : Wiki 自动更新流程
GH->>WU : 向 main/master 推送 specs 变更
WU->>QA : 调用 qoder-action 更新 Wiki
QA-->>WU : 返回更新的 Wiki 内容
WU-->>GH : 更新项目 RepoWiki
...
- **增强特性**
  - **优化提示词工程**：改进了prompt的结构化和上下文传递，增强了AI对需求的理解能力
  - **增强上下文感知**：自动加载AGENTS.md和项目规范文件，提供更准确的项目背景信息
  - **改进输出格式**：标准化的细化结果格式，便于后续处理和跟踪
- 输出要求
  - 详细的客户需求细化分析
  - 明确的歧义点和待确认事项
  - 标准化的客户确认清单
...
AddLabel --> Refine["QoderAI/qoder-action@v0<br/>增强的需求细化助手"]
Refine --> LoadContext["加载AGENTS.md和项目规范"]
LoadContext --> Analyze["理解原始需求意图"]
...
- 关键步骤
  ...
  - 生成【变更评估报告】
- **增强特性**
  - **智能识别增强**：改进了关联Issue编号的提取算法，提高了识别准确性
  - **上下文优化**：增强了Spec文档的读取和分析能力，提供更全面的影响评估
  - **结构化输出**：标准化的变更评估报告格式，便于决策和跟踪
- 评估维度
  - 文档影响：需要更新的Spec文档列表
  - 代码影响：需要修改的代码文件和模块
...
CI->>CI : 智能提取关联原始需求编号
CI->>GH : 读取原始需求和Spec文档
CI->>QA : 调用增强的 qoder-action 评估变更影响
QA-->>CI : 返回结构化的影响评估报告
...
### Qoder Wiki 自动更新工作流（wiki-update.yml）**全新**
- 触发机制
  - 监听 push 事件，针对 main 或 master 分支
  - 路径过滤：仅当推送的文件路径匹配 docs/specs/** 时触发
- 执行环境
  - ubuntu-latest
- 权限模型
  - id-token: write（用于 OIDC 令牌签发）
  - contents: read（读取仓库内容）
  - issues: write（用于Wiki更新操作）
- 关键步骤
  - 使用 actions/checkout@v4 拉取源码
  - 使用 QoderAI/qoder-action@v0 执行 /assistant 指令
  - 读取 docs/specs/ 目录中的最新Spec文档
  - 提取功能设计、交互流程、数据模型和验收标准
  - 自动更新项目的 RepoWiki，确保开发团队获取最新需求上下文
- 重点关注
  - 新增或修改了哪些功能
  - 交互流程和数据模型的变化
  - 验收标准（开发完成后的验证依据）
...
sequenceDiagram
participant GH as "GitHub"
participant WU as "Wiki 更新工作流"
participant QA as "QoderAI 服务"
participant FS as "文件系统"
participant Wiki as "项目Wiki"
GH->>WU : 向 main/master 推送 specs 变更
WU->>WU : 检查路径匹配 docs/specs/**
WU->>FS : 读取最新 Spec 文档
WU->>QA : 调用 qoder-action 提取关键信息
QA-->>WU : 返回功能设计、交互流程、数据模型、验收标准
WU->>Wiki : 更新项目 Wiki 内容
Wiki-->>WU : Wiki 更新完成
WU-->>GH : 完成 Wiki 同步
```
...
- **增强工作流遵循 AGENTS.md 中的Issue标签体系和分支规范**
- **变更影响评估和自动Changelog功能完善了版本管理的自动化闭环**
- **新增的Wiki更新工作流实现了Spec文档到知识库的自动同步**
...
- 内部依赖
  ...
  - **wiki-update.yml 依赖Spec文档和Wiki更新权限**
...
- 权限依赖
  ...
  - **wiki-update.yml 需要 issues: write 权限以更新Wiki内容**
...
- 触发粒度
  ...
  - **Wiki更新仅在specs目录变更时触发，避免不必要的Wiki更新**
- 文档处理优化
  ...
  - **Wiki更新工作流通过路径过滤确保只处理相关的Spec文档变更**
- **增强工作流的性能优化**
  - **issue-refine.yml 使用优化的提示词工程减少AI处理时间**
  - **change-impact.yml 智能识别关联Issue减少人工查找时间**
  - **auto-changelog.yml 只在Release创建时执行，避免频繁的日志维护**
  - **wiki-update.yml 通过路径过滤确保只处理specs目录的变更**
...
- 未触发工作流
  ...
  - **验证push事件：wiki-update.yml 需要向 main 或 master 分支推送且路径匹配 docs/specs/**
- 鉴权失败
...
- **自动Changelog问题**
  ...
  - **验证Git历史记录是否完整**
- **Wiki更新问题**
  - **确认Spec文档变更是否正确推送到 main 或 master 分支**
  - **检查路径过滤条件是否正确匹配 docs/specs/**
  - **验证Wiki更新权限是否配置正确**
  - **检查Spec文档格式是否符合提取要求**
- 构建/Lint 不一致
...
- **增强的需求自动细化工作流通过优化的提示词工程和上下文感知实现智能化的需求管理**
- **增强的变更影响评估工作流提供精准的变更影响分析和决策支持**
- **自动Changelog工作流简化了版本发布和变更追踪流程**
- **全新的Wiki自动更新工作流实现了Spec文档到项目知识库的自动同步**
- 结合工程的 TypeScript、ESLint 与 Vite 配置，形成一致的本地与 CI 开发体验

本次更新进一步完善了整个软件开发生命周期的自动化闭环：从需求细化、变更评估、文档评审、代码审查、规格生成、Wiki同步到版本发布，形成了完整的智能化流水线。新增的Wiki更新工作流填补了文档同步的空白，确保开发团队始终能够获取最新的需求上下文。这些工作流通过统一的标签体系、增强的提示词工程和标准化的工作流程，显著提升了团队协作效率和项目管理水平。建议后续逐步引入缓存、路径过滤与更细粒度的权限控制，以提升稳定性与效率。
...
- GitHub Actions 工作流概览
  ...
  - **issue-refine.yml：增强的需求自动细化**
  - **change-impact.yml：增强的变更影响评估**
  - **auto-changelog.yml：自动Changelog生成**
  - **wiki-update.yml：Wiki自动更新**
- Issue 标签体系
...
- **增强工作流特性**
  ...
  - **增强的提示词工程：优化AI理解和响应能力**
  - **Wiki自动同步：Spec文档到知识库的无缝集成**
