...
D["MockRepository.ts"]
E["HttpRepository.ts"]
F["schema-engine/types.ts"]
G["schema-engine/SchemaPage.tsx"]
H["schema-engine/renderers/index.ts"]
end
subgraph "配置"
I["config/domains.config.ts"]
end
subgraph "领域: enterprise"
J["domains/enterprise/manifest.ts"]
K["domains/enterprise/schema.ts"]
L["domains/enterprise/types.ts"]
end
subgraph "领域: news"
M["domains/news/manifest.ts"]
end
A --> B
B --> C
C --> D
...
## 架构总览
下图展示了从应用启动到页面渲染的关键路径：App 挂载 Provider，DataProvider 装配仓库，SchemaPage 读取 Schema 并调用仓库获取数据，通过渲染器渲染列。
...
## 结论
通过“清单 + Schema + 渲染器 + 仓库”的分层设计，本项目实现了高内聚、低耦合的可扩展后台架构。新业务域的开发可以聚焦于类型、Schema 与少量交互逻辑，借助统一的渲染器与数据源抽象快速落地。建议在后续迭代中逐步完善 HTTP 仓库、缓存与监控能力，进一步提升稳定性与性能。
